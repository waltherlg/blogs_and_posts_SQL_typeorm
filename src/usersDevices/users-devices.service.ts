...
export class UsersDeviceService {
  ...
  async getCurrentDeviceDataForRefreshStrategy(
    userId: string,
    deviceId: string,
  ) {
    const currentDevice =
  ...
  async getActiveUserDevices(userId: string) {
    const foundDevices =
      await this.userDeviceQueryRepository.getActiveUserDevices(userId);
    return foundDevices;
  }
  ...
  async deleteAllUserDevicesExceptCurrent(userId, deviceId): Promise<boolean> {
    const isDevicesDeleted =
      await this.usersDeviceRepository.deleteAllUserDevicesExceptCurrent(
        userId,
        deviceId,
      );
