...
async forTelegramm(@Body() payload: TelegramUpdateMessage){
    console.log(payload);
    this.telegramAdapter.sendMessageToTelegramm(`Привет, ${payload.message.from.first_name} , ты мне только что написал ${payload.message.text}`, payload.message.from.id)
