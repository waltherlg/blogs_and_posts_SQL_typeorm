...
export class NewQuestionsTable1701172661349 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "CommentLikes" DROP COLUMN "isUserBanned"`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "CommentLikes" ADD "isUserBanned" boolean NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "CommentLikes" ADD "login" character varying`,
    );
  }
}
