...
export class PlayerStatisticQueryRepository {
  ...
  async getTopPlayers(
    mergedQueryParams: RequestTopPlayersQueryParamsModel,
  ): Promise<PaginationOutputModel<topPlayerOutputType>> {
    const queryBuilder =
      this.playerStatisticQueryRepository.createQueryBuilder('statistic');
    ...
    queryBuilder
      .select(['statistic', 'player.login'])
      .leftJoin('statistic.Users', 'player');

    queryBuilder.orderBy(
      `statistic.${sortQueryParam[0].sortBy}`,
      sortQueryParam[0].sortDir,
    );
