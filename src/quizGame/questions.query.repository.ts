...
import { QuestionDbType, Questions, questionOutputSaType } from "./quiz.game.types";
import { Repository } from "typeorm";
import { validate as isValidUUID } from 'uuid';
import { questionMapper } from "./helpers/questions.mapper";
import { PaginationOutputModel } from "src/models/types";
import { publishedStatusFixer, sortDirectionFixer } from "src/helpers/helpers.functions";
...
export class QuestionsQueryRepository {
    ...
    async getAllQuestionsForSa(mergedQueryParams): Promise<PaginationOutputModel<questionOutputSaType>>{
      const bodySearchTerm = mergedQueryParams.bodySearchTerm;
      const publishedStatus = publishedStatusFixer(mergedQueryParams.publishedStatus) ;
      ...
      const queryBuilder = this.questionsQueryRepository.createQueryBuilder('question');
      ...
      const outQuestions = questions.map((question) => questionMapper.forSa(question))
