...
export class RegisterationEmailResendingUseCase
  implements ICommandHandler<RegisterationEmailResendingCommand>
{
  constructor(
  ...
  async execute(command: RegisterationEmailResendingCommand): Promise<any> {
    ...
    const result = await this.usersRepository.refreshConfirmationData(
      refreshConfirmationData,
    );
    return result;
  }
