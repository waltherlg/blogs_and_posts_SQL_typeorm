...
export class NewPasswordSetUseCase
  implements ICommandHandler<NewPasswordSetCommand>
{
  constructor(
  ...
  async execute(command: NewPasswordSetCommand): Promise<boolean> {
    ...
    const result = await this.usersRepository.newPasswordSet(
      command.newPasswordSetDto.recoveryCode,
      newPasswordHash,
    );
